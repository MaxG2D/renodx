...
namespace FakeHDRGain {
        ...
        float GetLuminance(float color) { return renodx::color::y::from::BT709((color.x, color.x, color.x)); }
    ...
    return float2(ApplyFakeHDRGain(color.xxx, gainScale, threshold, saturationMultiplier).x,
                  ApplyFakeHDRGain(color.yyy, gainScale, threshold, saturationMultiplier).y);
    }
